...
# ==================== 主函数 ====================
VIDEO_PATH = "/home/verser/Videos/cxk.mp4"
...
def main():
    ...
    while True:
        ...
        # 修正 2: 逻辑重构
        # 如果是第一帧（重新读的），其实位置就是初始位置，不用 track
        if frame_idx == 0:
            ...
            if current_score <= 0.98:
                print(f"🛑 追踪丢失 (Score: {current_score:.4f}) <= 0.98，程序退出。")
        ...
        curr_fps = fps_counter / elapsed if elapsed > 0 else 0
        ...
        if frame_idx % 60 == 0:
            print(f"Frame {frame_idx} | FPS: {curr_fps:.1f}")

    cap.release()
    cv2.destroyAllWindows()
    print(f"✅ 完成: {frame_idx}帧 | 平均 FPS: {curr_fps:.1f}")
