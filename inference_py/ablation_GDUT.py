...
# ================= 1. 路径与模型配置 =================
IMAGE_ROOT   = "/home/verser/Downloads/Drone_25k/MAV_images"
ANNO_ROOT    = "/home/verser/Downloads/Drone_25k/MAV_annotation"
...
# ================= 5. 评估器 =================
class GDUT_Ablation_Evaluator:
    def __init__(self, seq_name):
        self.seq_name = seq_name
        self.img_dir  = os.path.join(IMAGE_ROOT, seq_name)
        # gdut_mav_01 -> gdut_mav_annotation_01
        anno_folder   = seq_name.replace("gdut_mav_", "gdut_mav_annotation_")
        self.anno_dir = os.path.join(ANNO_ROOT, anno_folder)

        if not os.path.exists(self.img_dir) or not os.path.exists(self.anno_dir):
            self.num_frames = 0
            return

        self.img_files = sorted([f for f in os.listdir(self.img_dir) if f.endswith(('.jpg', '.png'))])
        self.gt_bboxes = self._load_labelme_jsons()
        self.num_frames = min(len(self.img_files), len(self.gt_bboxes))

    def _load_labelme_jsons(self):
        json_files = sorted([f for f in os.listdir(self.anno_dir) if f.endswith('.json')])
        gt_list = []
        for j_file in json_files:
            with open(os.path.join(self.anno_dir, j_file), 'r') as f:
                data = json.load(f)
    ...
    def run_inference(self, yolo, tracker, config):
        ...
        for i in tqdm(range(self.num_frames), desc=f"  {self.seq_name}", leave=False):
            ...
            # 关闭追踪时，检测结果直接作为预测
            is_valid = tracking_state or (not use_tracker and bbox is not None)
            pred = [float(x) for x in bbox[:4]] if is_valid else [0, 0, 0, 0]
...
if __name__ == "__main__":
    ...
    sequences = ["gdut_mav_01", "gdut_mav_02", "gdut_mav_03",
                 "gdut_mav_04", "gdut_mav_05", "gdut_mav_06"]
    ...
    for cfg in ABLATION_CONFIGS:
        ...
        for seq in sequences:
            ev = GDUT_Ablation_Evaluator(seq)
            if ev.num_frames == 0:
    ...
    print("\n" + "="*70)
    print("📊 GDUT-MAV 消融实验汇总 (各配置均值)")
    print("="*70)
    print(df_mean.to_string(index=False))

    out_path = "ablation_GDUT_results.csv"
