...
def MOD2_global(frame1, frame2):
    ...
    for contour in contours:
        ...
        ratio = w / h
        if 16 < area < 3000 and 0.6 < ratio < 3:
            rect = (x, y, w, h)
...
def main():
    ...
    save_video = True
    # -----------------------------------------
    cv2.namedWindow("MOD2 Global Test", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("MOD2 Global Test", 640, 640)
    cap = cv2.VideoCapture(video_input)
    if not cap.isOpened():
        print(f"❌ 错误: 无法打开视频源 {video_input}")
        return
    ...
    if fps == 0: fps = 30 # 防止摄像头没有FPS信息


    print("🚀 开始运行 MOD2_global 测试...")
    ...
    try:
        while True:
            ...
            # --- 核心调用 ---
            # 传入前一帧和当前帧进行检测
            # 注意：这里传入 copy 防止函数内部修改了原始画面用于显示
            try:
                rect = MOD2_global(prev_frame.copy(), curr_frame.copy())
            ...
            # 显示和保存
            cv2.imshow("MOD2 Global Test", display_frame)

            # 更新前一帧
            prev_frame = curr_frame.copy()

            # 按键检测
            key = cv2.waitKey(30) & 0xFF # 延时30ms，控制播放速度
            if key == ord('q'):
                print("用户退出。")
                break
